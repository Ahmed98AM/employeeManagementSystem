...
exports.signUp = catchAsync(async function (req, res, next) {
  // const group = req.body.group ? req.body.group.toLowerCase() : "employee";
...
exports.login = catchAsync(async function (req, res, next) {
  ...
  const foundUser = await User.findOne({ email }).select("+password +group");
  if (
    !foundUser ||
  ...
  if (foundUser.group !== "hr") {
    return next(new AppError("You are not eligible to sign in !"), 401);
  }
...
exports.restricTo = function (...group) {
  return (req, res, next) => {
    if (!group.includes(req.user.group)) {
      return next(
        new AppError("You do not have permission to perform this action !", 403)
      );
    }
